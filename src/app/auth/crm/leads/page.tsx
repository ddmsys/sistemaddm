...
export default function LeadsPage() {
  ...
  async function fetchLeads() {
    ...
    try {
      ...
      const leadsList = snapshot.docs.map(
  ...
  async function saveLead(e: FormEvent) {
    e.preventDefault();
    if (!editLead?.name || !editLead?.email) {
    ...
    try {
      if (editLead.id) {
        const leadRef = doc(db, "leads", editLead.id);
        const { id, ...leadData } = editLead;
        ...
        >
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setEditLead(
                  (prev) => ({ ...prev, name: e.target.value } as Lead)
                )
              }
              disabled={saving}
              required
          ...
          <div>
            ...
            <select
              className="w-full border px-2 py-1 rounded"
              value={editLead?.stage ?? "primeiro_contato"}
              onChange={(e) =>
                setEditLead(
                  (prev) => ({ ...prev, stage: e.target.value } as Lead)
                )
              }
              disabled={saving}
            >
              ...
              <option value="fechado_ganho">Fechado - Ganhou</option>
