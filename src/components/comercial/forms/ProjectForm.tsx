...
export function ProjectForm({
  ...
  const selectedTemplate = projectTemplates.find(
    (t) => t.productType === selectedCategory
  );

  // ================ OPTIONS ================

  const categoryOptions = Object.entries(PRODUCT_TYPE_LABELS).map(
    ([value, label]) => ({
      value,
      label: `${label} (${value})`,
    })
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              ...
              <div>
                ...
                <span className="ml-2 text-blue-900">
                  {selectedTemplate.estimatedDays} dias
                </span>
              </div>
              <div>
      ...
      <Card>
        ...
        <CardContent>
          <textarea
            className="w-full h-24 p-3 border border-gray-300 rounded-md focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
