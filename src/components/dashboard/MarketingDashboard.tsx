...
export function MarketingDashboard() {
  ...
  const creativesStatus = [
    ...
    },
    {
      status: 'review',
      count: 3,
      items: ['Vídeo Promocional 60s', 'Folder Impresso A4', 'Campanha Email Nurturing'],
    },
    {
      status: 'approved',
      count: 12,
  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <p className="text-slate-600">Campanhas, criativos e performance de marketing</p>
      </header>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 xl:grid-cols-3">
        {/* Campanhas Ativas */}
        <div className="rounded-lg border bg-white p-6 shadow-sm xl:col-span-2">
          <div className="mb-6 flex items-center justify-between">
            <h2 className="text-lg font-semibold text-slate-900">Campanhas Ativas</h2>
            <button className="rounded-md border border-blue-200 px-3 py-1 text-sm text-blue-600 hover:text-blue-800">
              Nova Campanha
            </button>
          </div>
              ...
              >
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-slate-500">{campaign.progress}% executado</p>
                  </div>
                </div>
        ...
        <div className="rounded-lg border bg-white p-6 shadow-sm">
          <h2 className="mb-6 text-lg font-semibold text-slate-900">Status dos Criativos</h2>
          <div className="space-y-4">
            {creativesStatus.map((stage) => (
              <div
                key={stage.status}
                className={`rounded-r-lg border-l-4 p-3 ${getCreativeStatusColor(stage.status)}`}
              >
                ...
                <div className="space-y-1">
                  ...
                  {stage.items.length > 3 && (
                    <p className="cursor-pointer text-xs text-slate-500 hover:text-slate-700">
        ...
        {/* Leads por Fonte */}
        <div className="rounded-lg border bg-white p-6 shadow-sm">
          <h2 className="mb-6 text-lg font-semibold text-slate-900">Leads por Fonte</h2>
          <div className="space-y-4">
        ...
        {/* Budget vs Spend */}
        <div className="rounded-lg border bg-white p-6 shadow-sm">
          <h2 className="mb-6 text-lg font-semibold text-slate-900">Orçamento vs Gasto</h2>
          <div className="space-y-4">
            <div>
              ...
              <div className="h-3 w-full rounded-full bg-slate-200">
                <div className="h-3 rounded-full bg-blue-500" style={{ width: '49.5%' }}></div>
              </div>
              <p className="mt-1 text-xs text-slate-500">49.5% do orçamento utilizado</p>
            </div>

            <div className="border-t pt-4">
              <h4 className="mb-3 text-sm font-medium">Por Canal:</h4>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  <th className="py-3 text-left font-medium text-slate-900">Canal</th>
                  <th className="py-3 text-left font-medium text-slate-900">Impressões</th>
                  <th className="py-3 text-left font-medium text-slate-900">Cliques</th>
                  <th className="py-3 text-left font-medium text-slate-900">CTR</th>
                  <th className="py-3 text-left font-medium text-slate-900">CPC</th>
                  <th className="py-3 text-left font-medium text-slate-900">Gasto</th>
                  <th className="py-3 text-left font-medium text-slate-900">Conversões</th>
                  <th className="py-3 text-left font-medium text-slate-900">ROI</th>
                </tr>
              </thead>
              <tbody>
                {channelPerformance.map((channel, idx) => (
                  <tr key={idx} className="border-b last:border-b-0 hover:bg-slate-50">
                    ...
                    <td className="py-3 text-slate-600">{channel.impressions}</td>
                    ...
                    <td className="py-3 font-medium text-emerald-600">{channel.roi}</td>
                  </tr>
                ))}
              </tbody>
