...
export function useFunnelData(period?: { start: Date; end: Date }) {
  const [rawFunnelData, setRawFunnelData] = useState<Omit<FunnelStage, 'percentage'>[]>([]);
  ...
  const funnelData = useMemo((): FunnelStage[] => {
    ...
    const totalCount = rawFunnelData.reduce((sum, stage) => sum + stage.count, 0);
  ...
  const stats = useMemo(() => {
    ...
    const wonDeals = funnelData.find((s) => s.stage === 'fechado_ganho')?.count || 0;
